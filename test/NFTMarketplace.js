...
describe("NFTMarketplace", function () {
  ...
  describe("Making marketplace items", function () {
    ...
    it("Track newly created item, transfer NFT from seller to marketplace and emit Offered event", async function () {
      ...
      await expect(
        marketplace.connect(addr1).makeItem(nft.address, 0, toWei(price))
      )
        .to.emit(marketplace, "Offered")
        .withArgs(1, nft.address, 0, toWei(price), addr1.address);
      // Owner of NFT should now be the marketplace
      expect(await nft.ownerOf(0)).to.be.equal(marketplace.address);
      ...
      expect(item.price).to.be.equal(toWei(price));
  ...
  describe("Purchasing marketplace items", function () {
    ...
    it("Update item as sold, pay seller, transfer NFT to buyer, charge fees and emit a Bought event", async function () {
      ...
      expect(await nft.ownerOf(0)).to.be.equal(addr2.address);
    });
    it("Fail for invalid item ids, sold items and when not enough ether is paid", async function () {
